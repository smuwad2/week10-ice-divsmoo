<!-- src/components/subcomponents/BlogPost.vue -->
<template>
...
export default {
  ...
  computed: {
    // Title: prefer explicit post.title, fallback to entry or 'Untitled'
    displayTitle() {
      if (!this.post) return "Untitled";
      return this.post.title ?? this.post.entry?.slice(0, 30) ?? "Untitled";
    },
    imageSrc() {
...
<style scoped>
.card { display:flex; gap:12px; padding:10px; border:1px solid #eee; border-radius:8px; background:#fff; }
.blog-post { align-items:flex-start; }
.mood-img { width:96px; height:96px; object-fit:cover; border-radius:6px; }
.body { flex:1; }
.title { margin:0 0 6px; font-size:1.05rem; font-weight:600; }
.entry { margin:0 0 8px; white-space:pre-wrap; }
